...
Camera::Camera(glm::vec3 position, float fieldOfView, float aspectRatio)
	: position(position), projectionMatrix(glm::perspective(glm::radians(fieldOfView), aspectRatio, 0.1f, 100.0f)), front(glm::vec3(0.0f, 0.0f, -1.0f)), groundLevel(position.y), yaw(-90.0f), pitch(0.0f) {
